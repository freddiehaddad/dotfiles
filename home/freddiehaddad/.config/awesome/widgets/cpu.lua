...
local CMD = [[sh -c "grep '^cpu.' /proc/stat; ps -eo 'pid:10,pcpu:5,pmem:5,comm:30,cmd' --sort=-pcpu ]] .. [[| grep -v [p]s | grep -v [g]rep | head -11 | tail -n +2"]]
...
local function worker(user_args)
	...
	local yellow = args.yellow or '#FFFF00'
	local red = args.red or '#FF0000'
